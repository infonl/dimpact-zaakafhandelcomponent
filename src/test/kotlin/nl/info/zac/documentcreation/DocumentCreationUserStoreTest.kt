...
class DocumentCreationUserStoreTest : BehaviorSpec({
    ...
    context("Recovering the user that started a document creation") {
        ...
        given("a token that was created longer ago than it stays valid") {
            val loggedInUser = createLoggedInUser()
            every { loggedInUserInstance.get() } returns loggedInUser

            `when`("the token is exchanged for a user after it expired") {
                val createdAt = Instant.parse("2026-01-01T10:00:00Z")
                val token = documentCreationUserStore.createToken(now = createdAt)
                val result = documentCreationUserStore.consumeUser(
                    token = token,
                    now = createdAt.plus(DocumentCreationUserStore.TOKEN_VALIDITY).plusSeconds(1)
                )

                then("it returns null, so an abandoned wizard URL cannot be used as that user") {
