...
class SmartDocumentsTemplateConverterTest : BehaviorSpec({
    ...
    Given("a REST request") {
        ...
        When("convert to JPA model is called") {
            val jpaModel = restTemplateRequest.toModel()
            ...
            Then("it produces a correct jpa representation") {
                ...
                with(jpaModel.first()) {
                    ...
                    templates.size shouldBe 2
                    templates.first().name shouldBe "root template 1"
                    templates.last().name shouldBe "root template 2"

                    children.size shouldBe 2
                    with(children.first()) {
                        name shouldBe "group 1"
                        templates.size shouldBe 2
                        templates.first().name shouldBe "group 1 template 1"
                        templates.last().name shouldBe "group 1 template 2"
                        parent!!.name shouldBe "root"
                        children shouldBe emptySet()
                    }
                    with(children.last()) {
                        name shouldBe "group 2"
                        templates.first().name shouldBe "group 2 template 1"
                        templates.last().name shouldBe "group 2 template 2"
                        parent!!.name shouldBe "root"
                        children shouldBe emptySet()
                    }
                }
