...
/**
 * This test creates a zaak, adds a task to complete the intake phase, closes the zaak with afleidingswijze 'ingangsdatum_besluit'.
 */
@Suppress("MagicNumber")
class ZaakRestServiceBrondatumAfleidingswijzeIngangsdatumBesluitArchiveTest : BehaviorSpec({
    ...
    given(
        ...
        lateinit var ingangsdatumBesluit: LocalDate
        val intakeId: Int
        ...
        itestHttpClient.performGetRequest(
            ...
            JSONArray(responseBody).let { resultaattypes ->
                (0 until resultaattypes.length())
                    ...
                    .first { it.getString("naam") == "Opgelegd - Ingang besluit" }
            }.run {
        ...
        itestHttpClient.performGetRequest(
            url = "$ZAC_API_URI/zaken/besluittypes/$ZAAKTYPE_CMMN_TEST_4_UUID",
        ...
        `when`("a besluit is added to the zaak") {
            ingangsdatumBesluit = LocalDate.now().plusDays(3)
            val vervaldatumBesluit = LocalDate.now().plusDays(21)
            val today = LocalDate.now()
            val publicationDate = today.plusMonths(2)
            val responseDate = today.plusMonths(3)

            logger.info { "Ingangsdatum besluit $ingangsdatumBesluit" }
        ...
        `when`("the zaak is completed with afhandelwijze 'Opgelegd - Ingang besluit' (afleidingswijze 'ingangsdatum_besluit'") {
            ...
            then("the zaak should be closed and have a result and startdatumBewaartermijn = ingangsdatum besluit") {
                zacClient.retrieveZaak(zaakUuid, RECORDMANAGER_1).let { response ->
                    ...
                    JSONObject(responseBody).run {
                        LocalDate.parse(getString("startdatumBewaartermijn")) shouldBe ingangsdatumBesluit
                    }
                }
            }
        }
