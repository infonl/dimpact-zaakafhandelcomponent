...
import nl.info.zac.itest.config.ItestConfiguration.OBJECT_PRODUCTAANVRAAG_3_INVALID_UUID
...
import nl.info.zac.itest.config.ItestConfiguration.ZAAK_PRODUCTAANVRAAG_3_INVALID_IDENTIFICATION
...
import nl.info.zac.itest.config.ItestConfiguration.ZAAK_PRODUCTAANVRAAG_3_UITERLIJKE_EINDDATUM_AFDOENING
...
class NotificationsTest : BehaviorSpec({
    ...
    Given(
        ...
        var response = zacClient.createZaaktypeCmmnConfiguration(
        ...
        lateinit var zaakProductaanvraagBrokenFlowUuid: UUID
        ...
        When(
            ...
            val response = itestHttpClient.performJSONPostRequest(
                ...
                requestBodyAsString = JSONObject(
                    mapOf(
                        "kanaal" to "objecten",
                        "resource" to "object",
                        "resourceUrl" to "$OBJECTS_BASE_URI/$OBJECT_PRODUCTAANVRAAG_3_INVALID_UUID",
                        "hoofdObject" to "$OBJECTS_BASE_URI/$OBJECT_PRODUCTAANVRAAG_3_INVALID_UUID",
                        "actie" to "create",
                        "aanmaakdatum" to ZonedDateTime.now(ZoneId.of("UTC")).toString(),
                        "kenmerken" to mapOf(
                            "objectType" to "$OBJECTS_BASE_URI/$OBJECTTYPE_UUID_PRODUCTAANVRAAG_DIMPACT"
            ...
            Then(
                ...
                // retrieve the newly created zaak and check the contents
                itestHttpClient.performGetRequest(
                    "$ZAC_API_URI/zaken/zaak/id/$ZAAK_PRODUCTAANVRAAG_3_INVALID_IDENTIFICATION"
                ).use { getZaakResponse ->
                    val responseBody = getZaakResponse.body.string()
                    logger.info { "Response: $responseBody" }
                    with(JSONObject(responseBody)) {
                        getString("identificatie") shouldBe ZAAK_PRODUCTAANVRAAG_3_INVALID_IDENTIFICATION
                        getJSONObject("zaaktype").getString("uuid") shouldBe ZAAKTYPE_TEST_4_UUID.toString()
                        getJSONObject("status").getString("naam") shouldBe "Intake"
                        // no group should be assigned - no behandelaar role in zaaktype
                        ...
                        // no initiator identificatie as BRP is turned off

                        zaakProductaanvraagBrokenFlowUuid = getString("uuid").let(UUID::fromString)
                    }
                }
            }
            ...
            And("an automated acknowledgement of receipt email is sent") {
                ...
                with(receivedMails) {
                    ...
                    with(getJSONObject(1)) {
                        ...
                        with(getString("mimeMessage")) {
                            // No BRP connection
                            shouldContain("From: sender@info.nl")
                            shouldContain("Return-Path: <sender@info.nl>")
                            shouldContain("Wij hebben uw verzoek ontvangen en deze op")
                        }
                    }
                }
        ...
        When("the get betrokkene endpoint is called for the zaak created from the productaanvraag") {
            val response = itestHttpClient.performGetRequest(
                url = "$ZAC_API_URI/zaken/zaak/$zaakProductaanvraagBrokenFlowUuid/betrokkene",
            )
            Then("the response should be a 200 HTTP response with a list consisting of the betrokkenen") {
                response.code shouldBe HTTP_OK
                val responseBody = response.body.string()
