...
class ZgwClientHeadersFactory @Inject constructor(
    ...
    /**
     * Sends [explanation] as the X-Audit-Toelichting of the next ZGW request made in [block]. It is also cleared when
     * [block] throws before that request is sent, so it cannot reach a later request on the same pooled thread.
     */
    fun <T> withAuditExplanation(explanation: String?, block: () -> T): T {
        if (explanation == null) return block()
        auditExplanation.set(explanation)
        return try {
            block()
        } finally {
            auditExplanation.remove()
