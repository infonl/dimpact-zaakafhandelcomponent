...
class ZaakRestService @Inject constructor(
    ...
    @PUT
    @Path("zaakdata")
    fun updateZaakdata(zaakDataUpdate: RestZaakDataUpdate) {
        val (zaak, zaakType) = zaakService.readZaakAndZaakTypeByZaakUUID(zaakDataUpdate.uuid)
        assertPolicy(policyService.readZaakRechten(zaak, zaakType).wijzigen)
        zaakVariabelenService.setZaakdata(zaakDataUpdate.uuid, zaakDataUpdate.zaakdata)
    }
