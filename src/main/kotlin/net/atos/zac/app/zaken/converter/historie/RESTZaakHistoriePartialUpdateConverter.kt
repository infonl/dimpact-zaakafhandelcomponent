...
class RESTZaakHistoriePartialUpdateConverter @Inject constructor(
    ...
    fun convertPartialUpdate(auditTrail: AuditTrailRegel, old: HashMap<*, *>, new: HashMap<*, *>) =
        old.getDiff(new).map { convertLine(auditTrail, it) }

    private fun convertLine(auditTrail: AuditTrailRegel, change: Map.Entry<Any, Pair<*, Any?>>): RESTHistorieRegel =
        RESTHistorieRegel(
            change.key.toString(),
