...
describe(ZaakBetrokkeneListComponent.name, () => {
  ...
  describe("ontkoppelen of a betrokkene", () => {
    ...
    it("deletes the betrokkene with the entered reden only once the dialog subscribes", async () => {
      const { openOntkoppelBetrokkene, deleteBetrokkeneMutation } =
        await setup();
    ...
    it("reports a failing ontkoppelen through the error handling of the mutation", async () => {
      ...
      const { openOntkoppelBetrokkene } = await setup();
      jest.spyOn(ZakenService.prototype, "deleteBetrokkene").mockReturnValue(
        fromPartial({
      ...
      await clickOntkoppelen();
      (
        openOntkoppelBetrokkene.mock.calls.at(-1)![1] as (
          reden: string,
        ) => Observable<unknown>
      )("fake-reden").subscribe({ error: () => undefined });
      await sleep();
