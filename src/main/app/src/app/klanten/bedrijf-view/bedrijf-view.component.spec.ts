...
describe(BedrijfViewComponent.name, () => {
  ...
  const routeDataSubject = new Subject<{ bedrijf: GeneratedType<"RestBedrijf"> | null }>();
  ...
  describe("initialisation", () => {
    ...
    it("sets vestigingsprofielOphalenMogelijk to true when bedrijf has vestigingsnummer", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf({ vestigingsnummer: "000011112222" }) });
      fixture.detectChanges();
    ...
    it("sets vestigingsprofielOphalenMogelijk to false when bedrijf has no vestigingsnummer", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf({ vestigingsnummer: undefined }) });
      fixture.detectChanges();
  ...
  describe("bedrijfsgegevens card", () => {
    ...
    it("renders bedrijfsnaam static-text field", () => {
      const element = fixture.debugElement.queryAll((de) => de.name === "zac-static-text")
  ...
  describe("kvknummer warning icon", () => {
    it("renders warning icon when kvkNummer is missing", () => {
      ...
      const kvkFields = fixture.debugElement.queryAll(
        (de) => de.name === "zac-static-text" && de.componentInstance.label === "kvknummer",
      );
      expect(kvkFields.length).toBe(1);
    });

    it("does not render warning icon when kvkNummer is present", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf({ kvkNummer: "12345678" }) });
      fixture.detectChanges();
      const kvkFields = fixture.debugElement.queryAll(
        (de) => de.name === "zac-static-text" && de.componentInstance.label === "kvknummer",
      );
  ...
  describe("profiel ophalen button", () => {
    ...
    it("calls ophalenVestigingsprofiel when clicked", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf() });
      fixture.detectChanges();
      jest.spyOn(klantenService, "readVestigingsprofiel").mockReturnValue(of(makeVestigingsprofiel()));
      const button = fixture.nativeElement.querySelector("button[mat-icon-button]") as HTMLButtonElement;
      button.click();
      fixture.detectChanges();
      expect(klantenService.readVestigingsprofiel).toHaveBeenCalledWith("000011112222");
    });
  });
  ...
  describe("ophalenVestigingsprofiel()", () => {
    ...
    it("populates vestigingsprofiel on success", () => {
      ...
      jest.spyOn(klantenService, "readVestigingsprofiel").mockReturnValue(of(profiel));
      component["ophalenVestigingsprofiel"]();
      fixture.detectChanges();
  ...
  describe("vestigingsprofiel fields", () => {
    ...
    it("hides adres field when vestigingsprofiel is loaded", () => {
      const adresField = fixture.debugElement.queryAll((de) => de.name === "zac-static-text")
        .find((de) => de.componentInstance.label === "adres");
  ...
  describe("zac-klant-zaken-tabel", () => {
    it("renders when bedrijf is available", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf() });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector("zac-klant-zaken-tabel")).toBeTruthy();
    });

    it("does not render when bedrijf is null", () => {
      routeDataSubject.next({ bedrijf: null });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector("zac-klant-zaken-tabel")).toBeFalsy();
    });
  });
  ...
  describe("zac-klant-contactmomenten-tabel", () => {
    it("renders when bedrijf has vestigingsnummer", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf({ vestigingsnummer: "000011112222" }) });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector("zac-klant-contactmomenten-tabel")).toBeTruthy();
    });

    it("does not render when bedrijf has no vestigingsnummer", () => {
      routeDataSubject.next({ bedrijf: makeBedrijf({ vestigingsnummer: undefined }) });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector("zac-klant-contactmomenten-tabel")).toBeFalsy();
    });
