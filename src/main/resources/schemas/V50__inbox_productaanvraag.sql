...
CREATE
    SEQUENCE ${schema}.sq_inbox_productaanvraag
START WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

CREATE
    INDEX idx_inbox_productaanvraag_id_initiator ON
    ${schema}.inbox_productaanvraag
        USING btree(id_initiator);

CREATE
    INDEX idx_inbox_productaanvraag_ontvangstdatum ON
    ${schema}.inbox_productaanvraag
        USING btree(ontvangstdatum);

CREATE
    INDEX idx_inbox_productaanvraag_productaanvraag_type ON
    ${schema}.inbox_productaanvraag
        USING btree(productaanvraag_type);
