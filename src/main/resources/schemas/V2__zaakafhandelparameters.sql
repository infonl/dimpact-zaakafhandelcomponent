...
CREATE
    SEQUENCE ${schema}.sq_zaakafhandelparameters
START WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

CREATE
    TABLE
        ${schema}.planitem_parameters(
            ...
            CONSTRAINT un_zaakafhandelparameters_planitem_definition UNIQUE(
                id_zaakafhandelparameters,
                id_planitem_definition
            ),
            CONSTRAINT fk_planitem_parameters_zaakafhandelparameters FOREIGN KEY(id_zaakafhandelparameters) REFERENCES ${schema}.zaakafhandelparameters(id_zaakafhandelparameters) MATCH SIMPLE ON
            UPDATE
                CASCADE ON
                DELETE
                    CASCADE
...
CREATE
    INDEX idx_planitem_parameters_id_zaakafhandelparameters ON
    ${schema}.planitem_parameters
        USING btree(id_zaakafhandelparameters);

CREATE
    SEQUENCE ${schema}.sq_planitem_parameters
START WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;
