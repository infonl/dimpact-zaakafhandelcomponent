...
CREATE
    SEQUENCE ${schema}.sq_inbox_document
START WITH
    1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

CREATE
    INDEX idx_inbox_document_id_document ON
    ${schema}.inbox_document
        USING btree(id_enkelvoudiginformatieobject);

CREATE
    INDEX idx_inbox_document_creatiedatum ON
    ${schema}.inbox_document
        USING btree(creatiedatum);

CREATE
    INDEX idx_inbox_document_titel ON
    ${schema}.inbox_document
        USING btree(titel);

CREATE
    INDEX idx_inbox_document_bestandsnaam ON
    ${schema}.inbox_document
