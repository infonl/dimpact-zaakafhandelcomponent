...
CREATE INDEX idx_template_group_name ON ${schema}.template_group USING btree (name);
CREATE INDEX idx_template_group_creation_date ON ${schema}.template_group USING btree (creation_date);
...
COMMENT ON COLUMN ${schema}.template_group.name IS 'Name of the template group';
COMMENT ON COLUMN ${schema}.template_group.parent_template_group_id IS 'ID of the parent template group (or NULL for root)';
COMMENT ON COLUMN ${schema}.template_group.creation_date IS 'Date on which the template group was stored in this table';
...
COMMENT ON COLUMN ${schema}.template.name IS 'Name of the template';
COMMENT ON COLUMN ${schema}.template.creation_date IS 'Date on which the template was stored in this table';
