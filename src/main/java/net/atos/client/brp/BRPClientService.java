...
public class BRPClientService {
    ...
    try {
      final var response =
          (RaadpleegMetBurgerservicenummerResponse)
              personenApi.personen(createRaadpleegMetBurgerservicenummerQuery(burgerservicenummer));
  ...
  private Optional<Persoon> handleFindPersoonAsync(
    ...
    if (!CollectionUtils.isEmpty(response.getPersonen())) {
      ...
      LOG.warning(
          () -> "Error while calling findPersoonAsync: %s".formatted(exception.getMessage()));
      return Optional.empty();
    }
  }
