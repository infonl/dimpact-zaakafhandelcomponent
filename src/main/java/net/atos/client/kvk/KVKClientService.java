...
public class KVKClientService {
  ...
  @Inject @RestClient private ZoekenClient zoekenClient;

  @Inject @RestClient private VestigingsprofielClient vestigingsprofielClient;
  ...
  public Resultaat list(final KVKZoekenParameters parameters) {
    try {
      ...
      LOG.warning(
          () ->
              ("Failed to search for company information using the KVK API: %s")
                  .formatted(exception));
    }
  ...
  public CompletionStage<Resultaat> listAsync(final KVKZoekenParameters parameters) {
    return zoekenClient.getResultsAsync(parameters).handle(this::handleListAsync);
  }

  public Optional<Vestiging> findVestigingsprofiel(final String vestigingsnummer) {
    return Optional.of(
        vestigingsprofielClient.getVestigingByVestigingsnummer(vestigingsnummer, false));
  }
  ...
  public CompletionStage<Optional<ResultaatItem>> findVestigingAsync(
  ...
  private Optional<ResultaatItem> convertToSingleItem(final Resultaat resultaat) {
    return switch (resultaat.getTotaal()) {
      ...
      default ->
          throw new IllegalStateException("Too many results: %d".formatted(resultaat.getAantal()));
    };
  }
