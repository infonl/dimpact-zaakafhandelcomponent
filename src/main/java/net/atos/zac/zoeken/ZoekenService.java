...
public class ZoekenService {
  ...
  public ZoekResultaat<? extends ZoekObject> zoek(final ZoekParameters zoekParameters) {
    ...
    zoekParameters
        ...
        .forEach(
            (filterVeld, filterParameters) ->
                query.addFacetField(format("{!ex=%s}%s", filterVeld, filterVeld.getVeld())));
    ...
    query.addSort(
        "id",
        SolrQuery.ORDER
            .desc); // uniek veld, zodat resultaten (van dezelfde query) altijd in dezelfde volgorde
    // staan
              ...
              .map(
                  solrDocument -> {
                    final ZoekObjectType zoekObjectType =
                        ZoekObjectType.valueOf(String.valueOf(solrDocument.get("type")));
                    return solrClient
                        .getBinder()
                        .getBean(zoekObjectType.getZoekObjectClass(), solrDocument);
                  })
                ...
                facetField.getValues().stream()
                    ...
                    .forEach(
                        facet ->
                            waardes.add(
                                new FilterResultaat(
                                    facet.getName() == null ? LEEG.toString() : facet.getName(),
                                    facet.getCount())));
