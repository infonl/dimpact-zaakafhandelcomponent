...
public class IndexeerService {
  ...
  public IndexeerService() {
    ...
    solrClient =
        new Http2SolrClient.Builder(String.format("%s/solr/%s", solrUrl, SOLR_CORE)).build();
  }
    ...
    final long added =
        addToSolrIndex(
            entities.stream()
                .filter(
                    zoekIndexEntity ->
                        zoekIndexEntity.getStatus() == ADD || zoekIndexEntity.getStatus() == UPDATE)
  ...
  private ZoekObject convertToZoekObject(
    ...
    try {
      ...
      LOG.log(
          WARNING,
          "[%s] '%s': %s"
              .formatted(zoekIndexEntity.getType(), zoekIndexEntity.getObjectId(), e.getMessage()));
    }
    if (zoekObject == null) {
  ...
  private boolean markTakenForReindexing(final int page, final long numberOfTasks) {
    ...
    final List<Task> tasks =
        takenService.listOpenTasks(
            TaakSortering.CREATIEDATUM, SorteerRichting.DESCENDING, firstResult, TAKEN_MAX_RESULTS);
