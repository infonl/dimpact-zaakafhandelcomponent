...
public class WebSocketServerEndPoint {
    ...
    @Inject private SessionRegistry registry;
    ...
    public void open(final Session session, final EndpointConfig conf) {
        ...
        final LoggedInUser loggedInUser =
                httpSession != null
                        ? (LoggedInUser) httpSession.getAttribute(LOGGED_IN_USER_SESSION_ATTRIBUTE)
                        : null;
    ...
    public void processMessage(
            final SubscriptionType.SubscriptionMessage message, final Session session) {
        if (message != null) {
            LOG.fine(
                    () ->
                            String.format(
    ...
    public void log(final Session session, final Throwable e) {
        ...
        LOG.log(
                Level.INFO,
                String.format("WebSocket error for %s (%s)", user(session), message),
                e);
    }
    ...
    private void denyAccess(final Session session, final String reason) {
        ...
        try {
            // According to the RFC, this close reason should be used if the other reasons are not
            // applicable.
    ...
    private String user(final Session session) {
        final String medewerker =
                (String) session.getUserProperties().get(LOGGED_IN_USER_SESSION_ATTRIBUTE);
