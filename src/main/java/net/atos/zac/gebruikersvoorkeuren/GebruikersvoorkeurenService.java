...
public class GebruikersvoorkeurenService {
  ...
  public List<Zoekopdracht> listZoekopdrachten(final ZoekopdrachtListParameters listParameters) {
    ...
    if (StringUtils.isNotBlank(listParameters.getMedewerkerID())) {
      predicates.add(
          builder.equal(root.get(Zoekopdracht.MEDEWERKER_ID), listParameters.getMedewerkerID()));
    }
  ...
  public void updateTabelInstellingen(final TabelInstellingen tabelInstellingen) {
    final TabelInstellingen bestaandeTabelInstellingen =
        readTabelInstellingen(tabelInstellingen.getLijstID(), tabelInstellingen.getMedewerkerID());
  ...
  private boolean existsZoekopdracht(final Zoekopdracht zoekopdracht) {
    ...
    predicates.add(
        builder.equal(root.get(Zoekopdracht.MEDEWERKER_ID), zoekopdracht.getMedewerkerID()));
    predicates.add(
        builder.equal(
  ...
  public void addDashboardCard(final String medewerkerId, final DashboardCardInstelling card) {
    if (card.getSignaleringType() != null) {
      final SignaleringInstellingen instellingen =
          signaleringenService.readInstellingenUser(card.getSignaleringType(), medewerkerId);
