...
public class ReferentieTabelBeheerService {
  ...
  private static final String UNIQUE_CONSTRAINT =
      "Er bestaat al een referentietabel met de code \"%s\".";

  private static final String FOREIGN_KEY_CONSTRAINT =
  ...
  private String getUniqueCode(final int i, final List<ReferentieTabel> list) {
    ...
    if (list.stream().anyMatch(referentieTabel -> code.equals(referentieTabel.getCode()))) {
  ...
  public ReferentieTabel updateReferentieTabel(final ReferentieTabel referentieTabel) {
    ...
    referentieTabelService
        .findReferentieTabel(referentieTabel.getCode())
        .ifPresent(
            existing -> {
              if (!existing.getId().equals(referentieTabel.getId())) {
                throw new FoutmeldingException(
                    String.format(UNIQUE_CONSTRAINT, referentieTabel.getCode()));
              }
            });
  ...
  public void deleteReferentieTabel(final long id) {
    ...
    final CriteriaQuery<HumanTaskReferentieTabel> query =
        builder.createQuery(HumanTaskReferentieTabel.class);
    ...
    final List<HumanTaskReferentieTabel> resultList =
