...
public class ZaakafhandelParameterBeheerService {
  ...
  public Optional<UUID> findZaaktypeUUIDByProductaanvraagType(final String productaanvraagType) {
    ...
    if (!resultList.isEmpty()) {
      if (resultList.size() > 1) {
        LOG.warning(
            String.format(
                "Er zijn meerdere zaaktypes gevonden voor productaanvraag type: '%s'",
                productaanvraagType));
      }
  ...
  private void mapZaakbeeindigGegevens(
    ...
    nieuweZaakafhandelParameters.setNietOntvankelijkResultaattype(
        mapVorigResultaattypeOpNieuwResultaattype(
            nieuweResultaattypen, vorigeZaakafhandelparameters.getNietOntvankelijkResultaattype()));
        ...
        .forEach(
            vorigeZaakbeeindigParameter -> {
              final UUID nieuwResultaattypeUUID =
                  mapVorigResultaattypeOpNieuwResultaattype(
                      nieuweResultaattypen, vorigeZaakbeeindigParameter.getResultaattype());
  ...
  private UUID mapVorigResultaattypeOpNieuwResultaattype(
    ...
    return nieuweResultaattypen.stream()
        .filter(
            resultaattype ->
                resultaattype.getOmschrijving().equals(vorigResultaattype.getOmschrijving()))
        .findAny()
