...
import static net.atos.zac.flowable.FlowableHelper.FlowableHelperProvider;
...
public class ZacCreateUserTaskInterceptor implements CreateUserTaskInterceptor {
    ...
    public void afterCreateUserTask(final CreateUserTaskAfterContext context) {
        ...
        FlowableHelperProvider.getInstance().getEventingService().send(screenEvent);
        FlowableHelperProvider.getInstance().getIndexeerService().addOrUpdateTaak(context.getTaskEntity().getId());
    }
}
