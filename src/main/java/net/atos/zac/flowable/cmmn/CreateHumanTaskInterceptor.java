...
public class CreateHumanTaskInterceptor
  ...
  public void beforeCreateHumanTask(final CreateHumanTaskBeforeContext context) {
    final String owner =
        (String) context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_OWNER);
    if (owner != null) {
      context.setOwner(owner);
    }
    final String candidateGroupId =
        (String)
            context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_CANDIDATE_GROUP);
    if (candidateGroupId != null) {
      context.setCandidateGroups(List.of(candidateGroupId));
    }
    final String assignee =
        (String) context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_ASSIGNEE);
    if (assignee != null) {
  ...
  public void afterCreateHumanTask(final CreateHumanTaskAfterContext context) {
    final Map<String, String> taakdata =
        (Map<String, String>)
            context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_TAAKDATA);
    ...
    context
        ...
        .setDueDate(
            (Date)
                context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_DUE_DATE));
    context
        .getTaskEntity()
        .setDescription(
            (String)
    ...
    final UUID zaakUUID =
        (UUID) context.getPlanItemInstanceEntity().getTransientVariable(VAR_TRANSIENT_ZAAK_UUID);
