...
public class ProductaanvraagService {
  ...
  public void verwerkProductaanvraag(final URI productaanvraagUrl) {
    ...
    if (zaaktypeUUID.isPresent()) {
      try {
        LOG.fine(
            () ->
                "Start zaak met CMMN case. Zaaktype: %s".formatted(zaaktypeUUID.get().toString()));
      ...
      if (zaaktype.isPresent()) {
        try {
          ...
          LOG.fine(
              () ->
                  "Start zaak met BPMN proces. Zaaktype: %s".formatted(zaaktype.get().toString()));
  ...
  private void warning(
      final String type, final ProductaanvraagDenhaag productaanvraag, final RuntimeException ex) {
    LOG.log(
        Level.WARNING,
        message(
            productaanvraag,
  ...
  public Map<String, Object> getFormulierData(final ORObject productaanvraagObject) {
    ...
    ((Map<String, Object>)
            productaanvraagObject.getRecord().getData().get(PRODUCT_AANVRAAG_FORMULIER_DATA_VELD))
  ...
  private void addInitiator(final String bsn, final URI zaak, final URI zaaktype) {
    ...
    final RolNatuurlijkPersoon rolNatuurlijkPersoon =
        new RolNatuurlijkPersoon(zaak, initiator, ROL_TOELICHTING, new NatuurlijkPersoon(bsn));
  ...
  private void registreerZaakMetCMMNCase(
    ...
    zaak.setOmschrijving(
        (String)
            formulierData.get(FORMULIER_KLEINE_EVENEMENTEN_MELDING_EIGENSCHAPNAAM_NAAM_EVENEMENT));
    zaak.setToelichting(
        (String)
            formulierData.get(
                FORMULIER_KLEINE_EVENEMENTEN_MELDING_EIGENSCHAPNAAM_OMSCHRIJVING_EVENEMENT));
  ...
  private void toekennenZaak(final Zaak zaak, final ZaakafhandelParameters zaakafhandelParameters) {
    ...
    if (zaakafhandelParameters.getGebruikersnaamMedewerker() != null) {
      ...
      zrcClientService.createRol(
          creeerRolMedewerker(zaakafhandelParameters.getGebruikersnaamMedewerker(), zaak));
    }
  }
