...
public final class OpenZaakPaginationUtil {
    ...
    public static int calculateOpenZaakPageNumber(final Pagination pagination) {
        return pagination.getPageNumber() * pagination.getPageSize() / PAGE_SIZE_OPEN_ZAAK
                + FIRST_PAGE_NUMBER_ZGW_APIS;
    }
     ...
     */
    public static <T> List<T> filterPageFromOpenZaakResult(
            final List<T> results, final Pagination pagination) {
        final int absolutePageNumber =
                pagination.getPageNumber() * pagination.getPageSize() / PAGE_SIZE_OPEN_ZAAK;
        final int relativePageNumber =
                pagination.getPageNumber()
                        - (absolutePageNumber * PAGE_SIZE_OPEN_ZAAK / pagination.getPageSize());
