...
public class RESTBAGObjectJsonbDeserializer implements JsonbDeserializer<RESTBAGObject> {
    ...
    @Override
    public RESTBAGObject deserialize(
            final JsonParser parser, final DeserializationContext ctx, final Type rtType) {
        final JsonObject jsonObject = parser.getObject();
        final BAGObjectType type =
                BAGObjectType.valueOf(jsonObject.getJsonString("bagObjectType").getString());

        return switch (type) {
            case ADRES -> JSONB.fromJson(jsonObject.toString(), RESTBAGAdres.class);
            case NUMMERAANDUIDING ->
                    JSONB.fromJson(jsonObject.toString(), RESTNummeraanduiding.class);
            ...
            case ADRESSEERBAAR_OBJECT ->
                    JSONB.fromJson(jsonObject.toString(), RESTAdresseerbaarObject.class);
        };
    }
}
