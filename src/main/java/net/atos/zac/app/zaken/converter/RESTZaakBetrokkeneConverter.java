...
public class RESTZaakBetrokkeneConverter {
        ...
        switch (rol.getBetrokkeneType()) {
            case NATUURLIJK_PERSOON -> {
                ...
                betrokkene.identificatie =
                        rolNatuurlijkPersoon.getBetrokkeneIdentificatie().getInpBsn();
            }
            case NIET_NATUURLIJK_PERSOON -> {
                final RolNietNatuurlijkPersoon rolNietNatuurlijkPersoon =
                        (RolNietNatuurlijkPersoon) rol;
                betrokkene.identificatie =
                        rolNietNatuurlijkPersoon.getBetrokkeneIdentificatie().getInnNnpId();
            }
            case VESTIGING -> {
                final RolVestiging rolVestiging = (RolVestiging) rol;
                betrokkene.identificatie =
                        rolVestiging.getBetrokkeneIdentificatie().getVestigingsNummer();
            }
            case ORGANISATORISCHE_EENHEID -> {
                final RolOrganisatorischeEenheid rolOrganisatorischeEenheid =
                        (RolOrganisatorischeEenheid) rol;
                betrokkene.identificatie =
                        rolOrganisatorischeEenheid.getBetrokkeneIdentificatie().getNaam();
            }
            case MEDEWERKER -> {
                final RolMedewerker rolMedewerker = (RolMedewerker) rol;
                betrokkene.identificatie =
                        rolMedewerker.getBetrokkeneIdentificatie().getIdentificatie();
            }
        }
        return betrokkene;
    }
    ...
    public List<RESTZaakBetrokkene> convert(final Stream<Rol<?>> rollen) {
        return rollen.map(this::convert).toList();
    }
}
