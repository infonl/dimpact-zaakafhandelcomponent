...
public class RESTHumanTaskParametersConverter {
  ...
  @Inject private RESTHumanTaskReferentieTabelConverter restHumanTaskReferentieTabelConverter;
  ...
  public List<RESTHumanTaskParameters> convertHumanTaskParametersCollection(
    ...
    return humanTaskDefinitions.stream()
        .map(
            humanTaskDefinition ->
                convertHumanTaskDefinition(humanTaskDefinition, humanTaskParametersCollection))
        .toList();
  }
  ...
  private RESTHumanTaskParameters convertHumanTaskDefinition(
    ...
    return humanTaskParametersCollection.stream()
        .filter(
            humanTaskParameters ->
                humanTaskParameters.getPlanItemDefinitionID().equals(humanTaskDefinition.id))
        .findAny()
        .map(
            humanTaskParameters ->
                convertToRESTHumanTaskParameters(humanTaskParameters, humanTaskDefinition))
  ...
  private RESTHumanTaskParameters convertToRESTHumanTaskParameters(
    ...
    restHumanTaskParameters.referentieTabellen =
        convertReferentieTabellen(humanTaskParameters, humanTaskDefinition);
  ...
  private List<RESTHumanTaskReferentieTabel> convertReferentieTabellen(
      final HumanTaskParameters humanTaskParameters,
      final RESTPlanItemDefinition humanTaskDefinition) {
    final List<RESTHumanTaskReferentieTabel> referentieTabellen =
        restHumanTaskReferentieTabelConverter.convert(humanTaskParameters.getReferentieTabellen());
    DefaultHumanTaskFormulierKoppeling.readFormulierVeldDefinities(humanTaskDefinition.id).stream()
        .filter(
            veldDefinitie ->
                referentieTabellen.stream()
                    .noneMatch(
                        referentieTabel -> veldDefinitie.name().equals(referentieTabel.veld)))
  ...
  private RESTHumanTaskParameters convertToRESTHumanTaskParameters(
    ...
    restHumanTaskParameters.formulierDefinitieId =
        DefaultHumanTaskFormulierKoppeling.readFormulierDefinitie(humanTaskDefinition.id).name();
    return restHumanTaskParameters;
  }
  ...
  public List<HumanTaskParameters> convertRESTHumanTaskParameters(
    ...
    return restHumanTaskParametersList.stream().map(this::convertRESTHumanTaskParameters).toList();
  }

  private HumanTaskParameters convertRESTHumanTaskParameters(
      final RESTHumanTaskParameters restHumanTaskParameters) {
