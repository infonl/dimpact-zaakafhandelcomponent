...
public class RESTTaakHistorieConverter {
    ...
    public List<RESTTaakHistorieRegel> convert(
            final List<HistoricTaskLogEntry> historicTaskLogEntries) {
        return historicTaskLogEntries.stream().map(this::convert).filter(Objects::nonNull).toList();
    }
    ...
    private RESTTaakHistorieRegel convert(final HistoricTaskLogEntry historicTaskLogEntry) {
        final RESTTaakHistorieRegel restTaakHistorieRegel =
                switch (historicTaskLogEntry.getType()) {
                    case USER_TASK_DESCRIPTION_CHANGED ->
                            convertValueChangeData(
                                    TOELICHTING_ATTRIBUUT_LABEL, historicTaskLogEntry.getData());
                    case USER_TASK_ASSIGNEE_CHANGED_CUSTOM ->
                            convertValueChangeData(
                                    BEHANDELAAR_ATTRIBUUT_LABEL, historicTaskLogEntry.getData());
                    case USER_TASK_GROUP_CHANGED ->
                            convertValueChangeData(
                                    GROEP_ATTRIBUUT_LABEL, historicTaskLogEntry.getData());
                    default ->
                            convertData(
                                    HistoricTaskLogEntryType.valueOf(
                                            historicTaskLogEntry.getType()),
                                    historicTaskLogEntry.getData());
                };
        if (restTaakHistorieRegel != null) {
            restTaakHistorieRegel.datumTijd =
                    convertToZonedDateTime(historicTaskLogEntry.getTimeStamp());
        }
        return restTaakHistorieRegel;
    }

    private RESTTaakHistorieRegel convertData(
            final HistoricTaskLogEntryType type, final String data) {
        return switch (type) {
    ...
    private RESTTaakHistorieRegel convertValueChangeData(
            final String attribuutLabel, final String data) {
        final TakenService.ValueChangeData valueChangeData =
                JSONB.fromJson(data, TakenService.ValueChangeData.class);
    ...
    private RESTTaakHistorieRegel convertOwnerChanged(final String data) {
        final AssigneeChangedData assigneeChangedData =
                JSONB.fromJson(data, AssigneeChangedData.class);
        return new RESTTaakHistorieRegel(
    ...
    private RESTTaakHistorieRegel convertDuedateChanged(final String data) {
        final DuedateChangedData duedateChangedData =
                JSONB.fromJson(data, DuedateChangedData.class);
        return new RESTTaakHistorieRegel(
