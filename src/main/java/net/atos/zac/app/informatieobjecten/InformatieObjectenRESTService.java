...
public class InformatieObjectenRESTService {
  ...
  @Inject private EnkelvoudigInformatieObjectUpdateService enkelvoudigInformatieObjectUpdateService;
  ...
  public List<RESTEnkelvoudigInformatieobject> listEnkelvoudigInformatieobjecten(
    ...
    final Zaak zaak =
        zoekParameters.zaakUUID != null ? zrcClientService.readZaak(zoekParameters.zaakUUID) : null;
    if (zoekParameters.informatieobjectUUIDs != null) {
      ...
      if (zoekParameters.besluittypeUUID != null) {
        ...
        final List<UUID> compareList =
            besluittype.getInformatieobjecttypen().stream().map(UriUtil::uuidFromURI).toList();
        return enkelvoudigInformatieobjectenVoorZaak.stream()
            .filter(
                enkelvoudigInformatieObject ->
                    compareList.contains(enkelvoudigInformatieObject.informatieobjectTypeUUID))
            .toList();
      } else {
            ...
            .orElseThrow(
                () ->
                    new RuntimeException(
                        String.format(
                            "No informatieobjecttype with omschrijving '%s' found "
                                + "for zaaktype '%s'",
                            INFORMATIEOBJECTTYPE_OMSCHRIJVING_BIJLAGE, zaak.getZaaktype())));

    final DocumentCreatieGegevens documentCreatieGegevens =
        new DocumentCreatieGegevens(zaak, restDocumentCreatieGegevens.taskId, informatieObjectType);
  ...
  public List<String> listZaakIdentificatiesForInformatieobject(
    ...
    return zaakInformatieobjects.stream()
        .map(
            zaakInformatieobject ->
                zrcClientService.readZaak(zaakInformatieobject.getZaak()).getIdentificatie())
        .toList();
  }
