...
Given("Resident {string} fills in the indienen-aansprakelijkheid-behandelen open-forms form", { timeout: ONE_MINUTE_IN_MS }, async function (this: CustomWorld, profileType: z.infer<typeof profilesSchema>) {
    ...
    await this.page.locator('#e67qrl9-demovoornaam').fill(profile.personalDetails.firstName + `:e2eid=${id}`);
    await this.page.locator('#e5c2pwf-demovoorletters').click();
    await this.page.locator('#e5c2pwf-demovoorletters').fill(profile.personalDetails.initials);
    await this.page.locator('#ew8j534-demotussenvoegsels').click();
    await this.page.locator('#ew8j534-demotussenvoegsels').fill(profile.personalDetails.prefix);
    await this.page.locator('#efjsdnh-demoachternaam').click();
    await this.page.locator('#efjsdnh-demoachternaam').fill(profile.personalDetails.lastName);
    await this.page.getByRole('button', { name: 'Volgende' }).click();

    // Incident details
    await this.page.getByLabel('Omschrijving van het voorval').click();
    await this.page.getByLabel('Omschrijving van het voorval').fill(profile.incidentDetails.description);
    await this.page.getByRole('textbox', { name: 'dd-MM-yyyy HH:mm' }).click();
    await this.page.getByRole('textbox', { name: 'dd-MM-yyyy HH:mm' }).fill(profile.incidentDetails.date);
    await this.page.getByLabel('materiële schade aan een').check();
    await this.page.getByLabel('ja', { exact: true }).check();
    await this.page.waitForSelector('#ej3ph74-hoeveelGetuigen', {'state': 'attached'});
    await this.page.getByText('Hoeveel getuigen?', { exact: true })
    await this.page.locator('#ej3ph74-hoeveelGetuigen').evaluate(node => node.click())
    ...
    await this.page.getByLabel('Nadere omschrijving van de').fill(profile.incidentDetails.location.furtherDescription);
    await this.page.getByLabel('U kunt hier aangeven waarom').click();
    await this.page.getByLabel('U kunt hier aangeven waarom').fill(profile.incidentDetails.reasonForMunicipalityLiability);
    await this.page.getByRole('button', { name: 'Volgende' }).click();

    // Damage details
    await this.page.getByLabel('Omschrijving schade voertuig').click();
    await this.page.getByLabel('Omschrijving schade voertuig').fill(profile.damageDetails.description);
    await this.page.getByLabel('Merk voertuig').click();
    await this.page.getByLabel('Merk voertuig').fill(profile.damageDetails.vehicle.make);
    await this.page.getByLabel('Kenteken voertuig').click();
    await this.page.getByLabel('Kenteken voertuig').fill(profile.damageDetails.vehicle.licensePlate);
    await this.page.getByLabel('Bedrijfsnaam').click();
    await this.page.getByLabel('Bedrijfsnaam').fill(profile.damageDetails.insurance.companyName);
    await this.page.getByLabel('Polisnummer').click();
    await this.page.getByLabel('Polisnummer').fill(profile.damageDetails.insurance.policyNumber);
    ...
    await this.page.getByLabel('Achternaam').fill(profile.witnessDetails[0].lastName);
    await this.page.getByLabel('Tussenvoegsels').click();
    await this.page.getByLabel('Tussenvoegsels').fill(profile.witnessDetails[0].prefix ?? '');
    await this.page.getByLabel('Voornamen').click();
    await this.page.getByLabel('Voornamen').fill(profile.witnessDetails[0].firstName ?? '');
    await this.page.getByPlaceholder('____ __').click();
    await this.page.getByPlaceholder('____ __').fill(profile.witnessDetails[0].postalCode ?? '');
    await this.page.getByLabel('Huisnummer').click();
    await this.page.getByLabel('Huisnummer').fill(profile.witnessDetails[0].houseNumber ?? '');
    await this.page.getByRole('button', { name: 'Volgende' }).click();
    ...
    await fileChooser2.setFiles(path.join(__dirname, profile.documents.invoice));
