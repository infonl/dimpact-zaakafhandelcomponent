...
function generateBrunoRequest(operationId, method, urlPath, operation, servers = [], openApiSpec = {}) {
    ...
    // Determine auth mode
    const authInfo = determineAuthMode(operation, openApiSpec);
 ...
 */
function updateBrunoCollection(parentFolder, openApiSpec, options = {}) {
    ...
    if (lastCollection && !dryRun) {
        const copied = copyEnvironments(lastCollection, collectionPath);
        if (copied) {
            log(`✓ Copied environments from: ${path.basename(lastCollection)}`, 'green');
        }
    } else if (lastCollection) {
        log(`[DRY RUN] Would copy environments from: ${path.basename(lastCollection)}`, 'cyan');
    } else {
    ...
    if (!dryRun) {
        ...
        // Create collection.bru with auth settings
...
`);
    ...
    try {
        ...
        const result = updateBrunoCollection(parentFolder, openApiSpec, options);
