...
| wijzigen                                      | X (zaak open, document onvergrendeld of vergrendeld door gebruiker) |             |       X       |           | OPA policy `document wijzigen`                                   |
...
Notes:
- `X (zaak open)` means that the zaak status must be 'Open' for the user role to have this right.
