...
| [Keycloak](https://www.keycloak.org/)       | Open Source Identity and Access Management product. | ZAC uses Keycloak for authentication and authorisation. ZAC authenticates to Keycloak using OIDC (OpenID Connect). ZAC also uses the Keycloak API to retrieve users and groups, for example to be able to assign zaken. |
