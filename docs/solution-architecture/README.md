...
- [Business Process Automation Architecture](businessProcessAutomationArchitecture.md) - The business process automation architecture of ZAC based on CMMN and BPMN.
...
- ZAC uses the [Flowable workflow engine](https://www.flowable.com/open-source) for process automation and supports both CMMN and BPMN to handle zaken.
...
| Standards                                    | <ul><li>ZAC runs on Kubernetes and provides a Kubernetes Helm Chart for deployment to a Kubernetes cluster.</li><li>ZAC does not offer an API to be used for other components. The ZAC backend API is solely meant to be used by the ZAC frontend and can therefore be seen as a ‘backend for the frontend’. OWASP and other security guidelines obviously still apply and are taken into account.</li></ul> |
