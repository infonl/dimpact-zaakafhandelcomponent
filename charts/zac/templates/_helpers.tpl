...
{{/*
The secret holding the Solr basic authentication credentials.
For an external Solr these are supplied through .Values.solr.username/.Values.solr.password and end up in
the ZAC secret; for a solr-operator managed Solr they are generated by the operator.
*/}}
{{- define "zaakafhandelcomponent.solr.basicAuthSecretName" -}}
{{- if .Values.solr.url -}}
{{ include "zaakafhandelcomponent.fullname" . }}
{{- else -}}
{{ include "zaakafhandelcomponent.solrcloud.fullname" . }}-solrcloud-basic-auth
{{- end -}}
{{- end }}

{{/*
The SOLR_USERNAME and SOLR_PASSWORD environment variables, read from the Solr basic authentication secret.
*/}}
{{- define "zaakafhandelcomponent.solr.credentialsEnv" -}}
- name: SOLR_USERNAME
  valueFrom:
    secretKeyRef:
      name: {{ include "zaakafhandelcomponent.solr.basicAuthSecretName" . }}
      key: {{ if .Values.solr.url }}SOLR_USERNAME{{ else }}username{{ end }}
- name: SOLR_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ include "zaakafhandelcomponent.solr.basicAuthSecretName" . }}
      key: {{ if .Values.solr.url }}SOLR_PASSWORD{{ else }}password{{ end }}
{{- end }}
