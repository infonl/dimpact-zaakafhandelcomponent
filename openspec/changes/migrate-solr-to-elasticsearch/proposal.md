...
- Replace the Solr `zac` core and its `managed-schema.xml` / `solrconfig.xml` with an Elasticsearch index and an index mapping/template covering the same fields (Dutch text analysis, dates, booleans, geo-point, dynamic `betrokkene` fields, copy/catch-all fields).
- Replace the versioned `SolrSchemaUpdate` migration machinery (`SolrSchemaV1`–`V7`, `SolrDeployerService`, `SolrSchemaUpdateHelper`, `FieldType`) with an Elasticsearch index-template / mapping bootstrap and reindex strategy.
- Re-implement faceted search (with per-facet exclusions), multi-field sorting, date-range filtering, geo filtering and full-text matching using Elasticsearch aggregations and query DSL — preserving the existing `/zoeken` REST contract and response shape (`RestZoekResultaat`: items + facets + pagination).
...
### New Capabilities
- `search-engine`: Indexing and full-text/faceted search of zaken, taken and documenten — covering index bootstrap, document indexing/reindexing, search queries with faceting, sorting, date-range and geo filtering, the `/zoeken` and `/indexeren` REST behaviour, and search backend readiness health checks. This captures the behaviour that must be preserved while the underlying engine moves from Solr to Elasticsearch.
...
- **Schema/config files**: remove `scripts/docker-compose/volume-data/solr-data/...` (`managed-schema.xml`, `solrconfig.xml`); add Elasticsearch index template/mapping.
