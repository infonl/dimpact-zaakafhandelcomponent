...
- [x] 7.1 Add/adapt unit tests for `IndexingService` and `SearchService` — **done as mockk unit tests** against a mocked `ElasticsearchClient` (bulk/delete/reindex routing; multi-index request, sort fallback, response mapping). Also added `ElasticsearchReadinessHealthCheckTest`. Deleted `SolrUtilTest`/`SolrDeployerServiceTest`/`SolrReadinessHealthCheckTest`. The full `./gradlew test` suite passes. (End-to-end Elasticsearch behaviour is covered by the existing `src/itest` suite — see below — rather than a separate testcontainers unit test.)
- [x] 7.2 Integration tests asserting result sets and ordering for representative queries — **covered by the existing `src/itest/.../SearchRestServiceTest`**, which creates zaken/taken/documenten, indexes them, and asserts the `/zoeken/list` and `/zoeken/zaken` result sets end-to-end against the Elasticsearch-backed ZAC app. The Dutch text analysis is configured in the per-type index mappings and exercised by these searches (no separate stemming-only assertion, matching the previous Solr test coverage).
- [x] 7.3 Tests for independent facet filtering — **done at two levels**: `SearchServiceTest` verifies the query construction (each facet aggregation re-applies the other facets' selections, excludes its own, selections as `post_filter`); `SearchRestServiceTest` asserts the per-field facet buckets returned in the `/zoeken/list` response against real Elasticsearch.
- [x] 7.4 Tests for date-range, geo filtering, sorting fallback, and the commit/refresh endpoint — sort fallback is unit-tested in `SearchServiceTest`; indexing + commit/refresh and searching are exercised end-to-end by `SearchRestServiceTest` (which waits for documents/taken to be indexed before asserting). Geo filtering has no REST input (see 4.6), so there is nothing to test.
- [x] 7.5 Verify the `/zoeken` and `/indexeren` REST contracts are unchanged — **preserved by design** (`RestZoekParameters`/`RestZoekResultaat` and the REST services are untouched) and asserted at the exact-JSON level by `SearchRestServiceTest` running against the Elasticsearch-backed stack.
