...
- **BREAKING**: `ZaakRestService.createZaak` REST endpoint (`POST /rest/zaken/zaak`) now returns only the zaak identification as a plain JSON string (e.g. `"ZAAK-2024-0000000001"`) instead of the full `RestZaak` object.
- The `createZaak` Kotlin function's return type changes from `RestZaak` to `String`.
- The final `policyService.readZaakRechten(...)` + `restZaakConverter.toRestZaak(...)` calls at the end of `createZaak` are removed since their output is no longer used; the function returns `zaak.identificatie` (from the ZGW `Zaak` created via `zgwApiService.createZaak`) directly.
- Frontend `ZaakCreateComponent.createZaakMutation`'s `onSuccess` handler is updated to receive the identification string directly instead of destructuring `{ identificatie }` from a `RestZaak`.
- Backend unit tests (`ZaakRestServiceTest.kt`) for `createZaak` are updated to assert the returned string instead of a `RestZaak`.
...
- Backend: `src/main/kotlin/nl/info/zac/app/zaak/ZaakRestService.kt` (`createZaak` function and REST endpoint).
