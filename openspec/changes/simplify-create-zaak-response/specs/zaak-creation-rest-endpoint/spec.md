...
The `POST /rest/zaken/zaak` endpoint SHALL, upon successfully creating a zaak, respond with only the zaak identification (`identificatie`) as a plain JSON string in the response body. It SHALL NOT include any other zaak fields (such as `uuid`, `groep`, `behandelaar`, `zaaktype`, `besluiten`, rights, or status) in the create response.

#### Scenario: Successful zaak creation returns the identification string
...
- **THEN** the response body is a JSON string containing the identification of the newly created zaak, and no other zaak field is present in the response body
