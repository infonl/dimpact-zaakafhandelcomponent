...
Rootless Podman does not support the privileged container Ryuk needs to run reliably — **confirmed empirically**: without `TESTCONTAINERS_RYUK_DISABLED=true` set as a real process environment variable, `ComposeContainer.start()` throws `ContainerLaunchException: Container startup failed for image testcontainers/ryuk:0.14.0` and the whole itest run fails. Critically, TestContainers reads this env var itself, independently of any application code — a value computed at runtime inside `ZacItestProjectConfig.kt` (e.g. by inspecting `DOCKER_HOST`) has no effect on whether TestContainers attempts to launch Ryuk; it can only affect ZAC's own teardown logic. The fix is to export the real env var before the JVM starts. Since the itest suite is most often run from IntelliJ (right-click "Run tests" on the `itest` folder, delegated to Gradle) rather than the CLI script, this now lives in `build.gradle.kts` itself — `detectPodmanDockerHost()` sets `DOCKER_HOST`/`TESTCONTAINERS_RYUK_DISABLED` on the `itest` `Test` task's `environment(...)` at configuration time, so it applies to the forked test JVM regardless of entry point (CLI, IntelliJ, CI). The CI workflow additionally exports it via `$GITHUB_ENV` before running `itest`, which the Gradle-level detection respects (it only auto-sets `DOCKER_HOST` when not already present in the environment).
- **Known side effect, fixed**: `ZacItestProjectConfig.kt`'s `afterProject()` used to gate its own explicit Compose teardown on this same `TESTCONTAINERS_RYUK_DISABLED` flag — a pre-existing coupling from before this migration, when the flag was only ever set manually (via `start-it-with-local-env.sh -c`) by a developer who wanted to keep containers running, so "Ryuk disabled" and "skip our teardown too" always meant the same thing. Now that Podman forces this flag `true` on every single run, that coupling meant every `itest` run left its containers running — confirmed as a real regression (repeated local runs all failed after the first, since the previous run's containers were still up). Fixed by introducing a separate, independent opt-in flag, `KEEP_ITEST_CONTAINERS_RUNNING`, for that use case; `afterProject()` now always tears down the Compose stack via TestContainers' own `.stop()` API (which needs no Ryuk) regardless of whether Ryuk itself was disabled.
...
- **[Risk, confirmed and fixed]** Disabling Ryuk removes automatic orphaned-container cleanup — under Podman this used to also disable ZAC's own explicit teardown, since both were driven by the same `TESTCONTAINERS_RYUK_DISABLED` flag (see Decision #2), so local Compose stacks persisted after every `itest` run and repeated local runs failed (the next run's containers collided with the still-running previous stack). → **Fixed**: introduced an independent `KEEP_ITEST_CONTAINERS_RUNNING` opt-in flag for developers who want that; by default `afterProject()` now always tears down the Compose stack regardless of Ryuk.
