...
- Let a BPMN process supply the vertrouwelijkheidaanduiding via a new required `vertrouwelijkheidsaanduiding` field on `SendEmailDelegate`, resolved and validated the same way as the delegate's existing fields, failing the service task when missing or invalid.
...
- `SendEmailDelegate`: reads the new `vertrouwelijkheidsaanduiding` process field (see below) and passes the resolved value explicitly — the one call site that overrides the default.
...
`PlanItemsRestService.doUserEventListenerPlanItem`'s two frontend callers (`zaak-afhandelen-dialog.component.ts`, `intake-afronden-dialog.component.ts`) hardcode `vertrouwelijkheidaanduiding: "OPENBAAR"` directly in their mutation payload object literal, next to their existing hardcoded `createDocumentFromMail: true` — the simplest way to keep these two flows fixed to `OPENBAAR` without adding any backend special-casing to a converter shared with the selectable mail-create flow.
...
Add `var vertrouwelijkheidsaanduiding: Expression? = null` to `SendEmailDelegate` (nullable, unlike the existing `lateinit var` fields for `from`/`to`/`template`) so a missing BPMN field produces a clear, thrown error instead of Kotlin's generic `UninitializedPropertyAccessException`. In `execute()`, resolve the expression and validate before sending, mirroring the delegate's existing pattern for its mail-template lookup exactly:
```kotlin
...
**Naming note**: the BPMN field is named `vertrouwelijkheidsaanduiding` (with the middle "s"), per the requirement text, even though the rest of the codebase (frontend key, `RestVertrouwelijkheidaanduiding`, `VertrouwelijkheidaanduidingEnum`) spells it without the "s". This is an intentional, requirement-specified exception scoped to the BPMN process-variable name only.

### No form-io schema or BPMN process changes for `SendEmailDelegate` itself
There is nothing in this codebase that enforces a Form.io form's required fields for a service task's own configuration — `SendEmailDelegate`'s fields (`to`, `from`, `template`, and now `vertrouwelijkheidsaanduiding`) are plain `<flowable:field>` values, resolved as `Expression`s. Whatever process wires up the service task itself is outside this repo — the user is supplying their own test BPMN process. This change's only responsibility is that the delegate itself refuses to proceed when the resolved value is missing or invalid, which also protects any already-deployed process that doesn't supply the field.

### New `ZAC_vertrouwelijkheidsaanduiding` Form.io custom field
Separately, a human-facing Form.io *task* form (a `userTask`'s own form, a different mechanism from the service task fields above) needs a way to offer the same 8 confidentiality levels as a dropdown, so its selected value can be carried as a process variable into `SendEmailDelegate`'s `vertrouwelijkheidsaanduiding` field downstream. `FormioSetupService` (`src/main/app/src/app/taken/taak-view/formio/formio-setup-service.ts`) gets a new `KNOWN_ZAC_FIELDS.VERTROUWELIJKHEIDSAANDUIDING = "ZAC_vertrouwelijkheidsaanduiding"` entry and a matching `initializeVertrouwelijkheidsaanduidingField` handler, following the exact pattern of the existing static/enum-like fields (`initializeZaakStatusField`, `initializeZaakResultField`): it sets `component.data.custom` to the same `VertrouwelijkaanduidingToTranslationKeyPipe.selectList` used by `zaak-create` and `mail-create`, translating each entry's label via `TranslateService.instant` up front (Form.io's own template interpolation doesn't run through the app's `translate` pipe), with `valueProperty = "value"` and `template = "{{ item.label }}"`. A form author opts in by setting a component's `ZAC_TYPE` attribute to `ZAC_vertrouwelijkheidsaanduiding` in their Form.io form definition (e.g. the user's own test intake form) — no other repo change is needed for this to work in any Form.io task form.

### Documentation
`docs/manuals/bpmn-guide/README.md`'s "Send email" section gets a new bullet under "add fields" for `vertrouwelijkheidsaanduiding`, and the XML example gets a corresponding `<flowable:field name="vertrouwelijkheidsaanduiding">` entry. The "Send confirmation email" section is left unchanged (no new field there).
