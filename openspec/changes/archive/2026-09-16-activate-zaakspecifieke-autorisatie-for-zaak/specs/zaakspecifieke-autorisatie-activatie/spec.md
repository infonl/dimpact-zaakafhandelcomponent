...
### Requirement: The werkvoorraad dialogs state that zaakspecifiek geautoriseerde zaken were left out

When a werkvoorraad verdelen or vrijgeven action leaves one or more of the selected zaken unprocessed
because they are zaakspecifiek geautoriseerd, the system SHALL tell the employee how many zaken were left
out and that this is the reason. The werkvoorraad lists open zaken only, so no other skip reason can occur
there.

#### Scenario: A selection containing zaakspecifiek geautoriseerde zaken
- **WHEN** an employee distributes or releases a selection of zaken in which some are zaakspecifiek
  geautoriseerd
