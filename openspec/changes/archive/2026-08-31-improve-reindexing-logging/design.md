...
- **The reindexed/error totals can drift on an installation with concurrent writes** → the total used
  ...
  created or deleted in the ZGW API while that reindex is still running therefore surfaces as a
  ...
  comparison. When more objects appear than the snapshot accounted for, the drift can push the raw
  difference negative; `reindexFinishedMessage()` clamps it to zero so the log never reports a
  negative error count. Documented here and on `reindexFinishedMessage()` so operators reading a
  nonzero "not reindexed because of errors" on an otherwise healthy, busy environment don't chase a
  phantom bug.
