...
- **Concrete examples, framed as "not recommended" vs "recommended" rather than "forbidden" vs "allowed"**: use fabricated but realistic illustrations rather than naming or implying any specific ZAC form is doing this today — the doc should teach the risk pattern without singling out (or fingerprinting) an actual production form. For `custom` component logic, the illustration is a postcode-lookup component that calls an external API directly from the browser (with an embedded API key), disables form validation, and force-submits — chosen because it's a plausible, recognizable scenario for municipality forms without being a real, traceable example. Pair each "not recommended" example with a safer alternative:
  ...
  - Custom component logic that reaches into `instance.root`, mutates global validation behavior (`root.shouldValidate`), calls external APIs with embedded credentials, or force-submits the form → flagged as high-risk regardless of alternative, since JSONLogic cannot express this and no safe drop-in replacement exists yet; beheerders are advised to keep such logic minimal, side-effect-free where possible, and to have it reviewed by someone who understands JavaScript before uploading the form.
...
- **Forward-looking upload-restriction warning**: state that ZAC may in future block uploading forms with JavaScript logic, and that JSON Logic forms will always remain allowed. This is phrased as a warning, not a commitment to a specific timeline or mechanism (no upload-time detection exists yet — that remains the deferred, developer-facing follow-up per AC3). Rationale: gives beheerders a concrete forward-looking incentive to choose JSON Logic for new forms now, reducing future migration work, without over-promising exactly when/how enforcement will land.
