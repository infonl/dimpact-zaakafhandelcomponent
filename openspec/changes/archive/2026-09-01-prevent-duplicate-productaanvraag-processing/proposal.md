...
- Wrap `ProductaanvraagService.handleProductaanvraag` so that it claims the productaanvraag object UUID before doing any zaak-creation work, skips processing entirely if the claim is rejected, and marks the record `DONE` immediately after the zaak has been created and its CMMN case or BPMN process has been started (not after the full `handleProductaanvraagDimpact` chain, which also does role assignment, document pairing, and — for CMMN — sending a confirmation email; see design.md for the exact call sites and the rationale).
- A stale claim (marked in-progress but never completed, e.g. due to a crash) is automatically reclaimable after a configurable TTL (default 10 minutes, overridable via the `PRODUCTAANVRAAG_CLAIM_TIMEOUT_MINUTES` environment variable and the corresponding ZAC Helm chart value), so a genuinely lost productaanvraag is not stuck forever without requiring manual intervention.
...
- **Configuration**: new environment variable `PRODUCTAANVRAAG_CLAIM_TIMEOUT_MINUTES` (default `10`), documented in `.env.example` and exposed as a ZAC Helm chart value (`charts/zac/values.yaml` / `charts/zac/templates/config.yaml`).
