...
- [x] 1.1 Add `behandelaarIsLoggedInUser: Boolean` to `ZaakData`, `TaakData` and `DocumentData` in
...
- [x] 1.2 Add `zaak_allowed if { zaak.behandelaarIsLoggedInUser }` (and the taak/document equivalents) to
...
- [x] 1.3 Populate `behandelaarIsLoggedInUser` in the three zoekobject-based `PolicyService` overloads
...
## 2. Search index: the medewerkers individually authorised for the zaak

- [x] 2.1 Add `zaakGeautoriseerdeMedewerkers` field constants to `ZoekObject`, `ZaakZoekObject`,
      ...
      type.
- [x] 2.2 Populate the new field in `ZaakZoekObjectConverter`, `TaakZoekObjectConverter` and
...
- [x] 2.3 Add `SolrSchemaV9` adding the four fields plus three copyFields, mirroring `SolrSchemaV8`, with
...
- [x] 2.4 Widen `SearchService.getZaakspecifiekGeautoriseerdFilterQuery()` so each per-zaaktype exclusion
      clause also excludes rows whose `zaakGeautoriseerdeMedewerkers` contain the logged-in user. Verify with
...
- [x] 3.1 Add a `ZrcClientService`-based helper alongside `isZaakspecifiekGeautoriseerd` in
...
- [x] 3.2 Add `isZaakspecifiekGeautoriseerd: Boolean?` to `RestZaakCreateData` with the tri-state semantics
...
- [x] 3.3 Add four exception types under `nl.info.zac.app.zaak.exception`, each extending
...
- [x] 3.4 Add the two exception types for the behandelaar restrictions (release refused, reassignment
      refused) the same way, with their own `ErrorCode` entries. Verify the same way as 3.3.
- [x] 3.5 Add the Dutch and English translations for every new error code to
...
- [x] 3.6 Implement the activation branch in `ZaakRestService.updateZaak`: evaluate all four preconditions
...
- [x] 3.7 Trigger a direct reindex of the zaak, its taken and its documenten on successful activation, rather
...
- [x] 4.1 Reject removing or changing the behandelaar of a zaakspecifiek geautoriseerde zaak in the
...
- [x] 4.2 Skip zaakspecifiek geautoriseerde zaken in `ZaakService.releaseZaken` and in the batch assignment
...
- [x] 4.3 Expose `isZaakspecifiekGeautoriseerd` on the werklijst row so the frontend can name the skip reason:
...
- [x] 4.4 Surface the refusal on the zaakdetailpagina: the confirmation/refusal dialog per the Figma design
...
- [x] 5.1 Add the activation control to `zaak-details-wijzigen.component`, shown only when the zaak's zaaktype
...
- [x] 5.2 Confirm no change is needed to the lock indicator: `zaak-details-card.component.html` already
...
- [x] 6.1 Map the recordmanager and beheerder functional roles to the `zaakspecifiek_geautoriseerd`
...
- [x] 6.2 Update `docs/solution-architecture/accessControlPolicies.md` with the current-behandelaar exception
...
- [x] 6.3 Add an integration test alongside the existing `*ZaakspecifiekAutorisatie*Test.kt` family covering
...
- [x] 6.4 Extend the integration tests to cover the refusals: activation without a behandelaar, activation on
...
- [x] 6.5 Add integration tests for the recordmanager/beheerder access path, which this change moves entirely
...
- [x] 6.6 Add an integration test for the recovery path of a flagged zaak whose behandelaar was removed
...
- [x] 6.7 Add the PABC mapping to the deployment notes as a prerequisite, and record the manual reindex step
...
- [x] 6.8 Run `./gradlew spotlessApply detektApply build` and `./scripts/lint-changed-files.sh`, and verify
