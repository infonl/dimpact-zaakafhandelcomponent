...
The OPA policies (`zaak-rechten.rego`, `taak-rechten.rego`, `document-rechten.rego`, `werklijst-rechten.rego`, `notitie-rechten.rego`, `overige-rechten.rego`) SHALL grant each permission directly to every application role (`raadpleger`, `behandelaar`, `coordinator`, `recordmanager`, `beheerder`, `zaakspecifiek_autorisatie_behandelaar`) that is allowed to perform it, without depending on a user also carrying any "lower" application role in the hierarchy `raadpleger < behandelaar < coordinator < recordmanager < beheerder`, and without `zaakspecifiek_autorisatie_behandelaar`'s grants depending on the user also holding `behandelaar` or `raadpleger`.
...
#### Scenario: A user with only the zaakspecifiek_autorisatie_behandelaar role, and no behandelaar or raadpleger role, has behandelaar-equivalent rights
- **WHEN** a user has only the `zaakspecifiek_autorisatie_behandelaar` application role (and not `raadpleger`, `behandelaar`, `coordinator`, `recordmanager`, or `beheerder`) in `user.rollen`
- **THEN** the `zaak-rechten` policy's `lezen` and `wijzigen` permissions SHALL evaluate to `true` for an open zaak of an allowed zaaktype, identical to what a user with only the `behandelaar` application role would get
...
#### Scenario: Permission table documents the zaakspecifiek geautoriseerde zaak restriction
- **WHEN** `accessControlPolicies.md` is read
- **THEN** it SHALL document that a checkmark for `behandelaar`, `raadpleger`, or `coordinator` on a zaak/taak/document permission does not apply to a zaakspecifiek geautoriseerde zaak (or its taken/documenten) unless that user also holds `zaakspecifiek_autorisatie_behandelaar` for the zaaktype, SHALL include `zaakspecifiek_autorisatie_behandelaar` in the application roles table with its description, and SHALL NOT claim that this change specifies or restricts `recordmanager`/`beheerder` access to a zaakspecifiek geautoriseerde zaak (left to a follow-up story)
