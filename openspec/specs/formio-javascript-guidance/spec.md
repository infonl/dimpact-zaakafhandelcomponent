# formio-javascript-guidance Specification

## Purpose
This capability documents the security guidance the ZAC BPMN handleiding and developer docs must provide about JavaScript in Form.io forms: JavaScript in `custom` component logic runs as trusted, unrestricted code in every user's browser, so beheerders should prefer Form.io's JSONLogic (or Simple Conditions) alternative wherever it is available, and should treat raw JavaScript as a last resort reserved for someone who understands the risk. It also covers linking to authoritative JSONLogic references, providing recommended vs. not-recommended examples, warning about possible future upload restrictions on JavaScript-containing forms, and cross-linking the guidance from `docs/development/bpmn.md`.
