# behandelaar-groups-single-zaaktype Specification

## Purpose
This capability documents the hierarchy-free authorization behavior of the `GET /rest/identity/zaaktype/{zaaktypeDescription}/behandelaar-groups` endpoint, which returns only the active groups explicitly authorised for the `behandelaar` application role for a single zaaktype.
